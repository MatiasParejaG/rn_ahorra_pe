...
export default function GrupoDetail() {
  ...
  const grupo = userGrupos.find((g: any) => g.$id === grupoId) as Grupo & { userRole: string; membershipId: string };
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="bg-white px-6 pt-4 pb-4 border-b border-gray-100">
        <View className="flex-row items-center justify-between mb-4">
          ...
          <Text className="text-xl font-bold text-gray-800">Detalles del Grupo</Text>
          <View className="w-6" />
        </View>
      </View>
              ...
              >
                {grupo.foto_bg ? (
                  ...
                  <View className="w-full h-full items-center justify-center">
                    <MaterialCommunityIcons name="account-group" size={64} color="white" />
                  </View>
                )}
              </View>
              ...
              <View className="p-5">
                <View className="flex-row items-start justify-between mb-3">
                  <View className="flex-1">
                    <Text className="text-2xl font-bold text-gray-800 mb-2">{grupo.nombre}</Text>
                    {grupo.descripcion && (
                      <Text className="text-sm text-gray-600">{grupo.descripcion}</Text>
                    )}
                  </View>
                  {isAdmin && (
                  ...
                  <View className="flex-row items-center flex-1">
                    ...
                    <View className="ml-3 flex-1">
                      ...
                      <Text className="text-xs text-gray-500">
                        {new Date(member.fecha_union).toLocaleDateString('es-PE', {
                          day: '2-digit',
                          month: 'short',
                          year: 'numeric',
                        })}
                ...
                >
                  {isLeaving ? (
                    ...
                    <>
                      ...
                      <Text className="text-red-600 font-semibold ml-2">Salir del Grupo</Text>
                    </>
                  )}
                </TouchableOpacity>
              )}
